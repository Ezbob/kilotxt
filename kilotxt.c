...
enum editorKey {
    ARROW_LEFT = 1000, /* starting at 1000 to be wide enough that it does not go into ASCII range */
    ARROW_RIGHT,
    ARROW_UP,
    ARROW_DOWN,
    PAGE_UP,
        ...
        if ( filerow >= E.numrows ) {
            if ( E.numrows == 0 && y == rows / 3 ) {
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome),
                        "Kilotxt editor -- version %s", KILOTXT_VERSION);
                if ( welcomelen > cols ) {
                    welcomelen = cols;
                } 
...
void editorRefreshScreen() {
    ...
    /* setting cursor position according to struct */
    char buf[32];
